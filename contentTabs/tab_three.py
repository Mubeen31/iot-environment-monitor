...
@app.callback(Output('hum2', 'children'),
              [Input('update_value3', 'n_intervals')])
def update_value(n_intervals):
    url = 'https://api.thingspeak.com/channels/2007583/fields/1/last.csv'
    df = pd.read_csv(url)
    current_temp = df['field1'].iloc[0]

    url = 'https://api.thingspeak.com/channels/2007583/fields/1.csv?days=1'
    df1 = pd.read_csv(url)
    ...
    today_max = df1[df1['Date'] == unique_date[-1]]['field1'].max()
    today_min = df1[df1['Date'] == unique_date[-1]]['field1'].min()

    url = 'https://api.thingspeak.com/channels/2007583/fields/1.csv?days=2'
    df2 = pd.read_csv(url)
    ...
    yesterday_max = df2[df2['Date'] == yesterday_unique_date[-2]]['field1'].max()
    yesterday_min = df2[df2['Date'] == yesterday_unique_date[-2]]['field1'].min()
    ...
    return [
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.Div('Today Max'),
                    html.Div('{0: .1f}%'.format(today_max))
                ], className='tab_temp_row')
            ], className='bg_container twelve columns')
        ], className='row'),
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.Div('Today Min'),
                    html.Div('{0: .1f}%'.format(today_min))
                ], className='tab_temp_row')
            ], className='bg_container twelve columns')
        ], className='row'),
        ...
        html.Div([
            html.Div([
                html.Div([
                    ...
                    html.Div('{0: .1f}%'.format(yesterday_max))
                ], className='tab_temp_row')
            ], className='bg_container twelve columns')
        ], className='row'),
        ...
        html.Div([
            html.Div([
                html.Div([
                    ...
                    html.Div('{0: .1f}%'.format(yesterday_min))
                ], className='tab_temp_row')
            ], className='bg_container twelve columns')
        ], className='row')
    ]
